...
int main(){
	...
	/**
	 * condition_variable:
	 * 如何使用？std::condition_variable类 搭配 std::mutex类 来使用，
	 * std::condition_variable对象的作用不是用来管理互斥量的，它的作用是用来同步线程，它的用法相当于编程中常见的flag标志。
	 * std::condition_variable，A、B两个人约定notify_one为行动号角，A就等着（调用wait(),阻塞，
	 * 只要B一调用notify_one，A就开始行动（不再阻塞）。
	 *
	 * wait(locker) : unlock 状态 -> 被 notify -> lock 状态
	 * wait函数需要传入一个std::mutex（一般会传入std::unique_lock对象）。
	 * wait函数会自动调用 locker.unlock() 释放锁（因为需要释放锁，所以要传入mutex）并阻塞当前线程，
	 * 本线程释放锁使得其他的线程得以继续竞争锁。一旦当前线程获得notify
